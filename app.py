...
app = dash.Dash(__name__, title='Posssibility Dash Apps Analytics')
...
# df['gross income'] = df['Rating'].apply(lambda x: '🔥' if x > 25 else '🚒')
...
def parse_contents(contents, filename, dt):
    # global df
    ...
    return html.Div([
        html.H5(filename),
        html.H6(datetime.datetime.fromtimestamp(dt)),
        dash_table.DataTable(
            ...
            #     for c in ['Rating', 'Total', 'cogs', 'City']
            # },
            # tooltip={
            #     c: {'value': markdown_table, 'type': 'markdown'}
            ...
            #     {
            #         'if': {'column_id': ["Date", "Time", "Total", "Gender", "Tax ", "Rating", "City"]},
            #         'textAlign': 'left'
            #     },
            #     {
            #         'if': {
            #             'filter_query': '{Unit price} >= 75 && {Unit price} < 95',
            #             'column_id': 'Unit price'
            #         },
            ...
            #             'filter_query': '{{Total}} = {}'.format(df1["Total"].max())
            #         },
            ...
            #
            #             # since using .format, escape { with {{
            #             'filter_query': '{{Rating}} = {}'.format(df1['Rating'].max())
            #         },
            ...
            #
            #             # since using .format, escape { with {{
            #             'filter_query': '{{Tax}} = {}'.format(df1['Tax'].max())
            #         },
            ...
            #     {
            #         'if': {
            #             'filter_query': '{gross income} > 19 && {Rating} < 6',
            #             'column_id': 'gross income'
            #         },
            #         'color': 'tomato',
            #         'fontWeight': 'bold'
            #     },
            #     {
            #         'if': {
            #             'filter_query': '{cogs} > 300',
            #             'column_id': 'cogs'
            #         },
            #         'textDecoration': 'underline'
            #     }
            #     # data_bars(df, 'Total') + data_bars(df, 'Rating'),
            #     # data_bars_diverging(df, 'cogs') + data_bars_diverging(df, 'gross income')
            # ],
            style_header={
...
#                          y=df.EU_Sales.mean(),
#                          xref="x", yref="y",
#                          text="High Average:<br> %.2f" % df.EU_Sales.mean(),
#                          ax=0, ay=-40),
#                     dict(x=df.EU_Sales.idxmax(),
#                          y=df.EU_Sales.max(),
#                          xref="x", yref="y",
#                          text="High Max:<br> %.2f" % df.EU_Sales.max(),
#                          ax=0, ay=-40)]
# low_annotations = [dict(x="2015-05-01",
#                         y=df.NA_Sales.mean(),
#                         xref="x", yref="y",
#                         text="Low Average:<br> %.2f" % df.NA_Sales.mean(),
#                         ax=-40, ay=40),
#                    dict(x=df.EU_Sales.idxmin(),
#                         y=df.NA_Sales.min(),
#                         xref="x", yref="y",
#                         text="Low Min:<br> %.2f" % df.NA_Sales.min(),
#                         ax=0, ay=40)]
...
fig6.update_xaxes(
    dtick="M1",
    tickformat="%b\n%Y")
# k = df["Total", "Tax", "cogs", "Rating", "gross income"]
# # df.columns = df[k]
# # print(df.columns)
# fig6 = px.line(df, x=df.Time, y=k,
...
#
# group_labels = ['Tax', 'Cogs', 'Quantity', 'Unit price']
#
# # Create distplot with custom bin_size
# fig13 = ff.create_distplot(hist_data, group_labels, bin_size=.2)

# x1 = df.grossincome
# x2 = df.Rating

# group_labels = ['gross income', ' Food Rating']
#
...
#
# fig14.update_layout(title_text='Customized Distplots for Grossincome Vs Rating')
...
app.layout = dbc.Container([
            ...
            dbc.Card([
                ...
                dbc.CardBody(
                    [
                        ...
                        # dcc.Dropdown(id="monday", options=[{"label:t", "value:t"} for t in tim], clearable=True,
                        #              style={"color": "#000000"}, value="morning"),
                    ]
                )
            ]),
    ...
    dbc.Row([
        dbc.Col([
            ...
            html.Div([
                dbc.Col([
                    ...
                    html.Legend("What type of Juice do you Prefer?"),
                    # html.Br(),
                    dcc.RadioItems(
                        id="rd2",
                        options=[
        ...
        dbc.Col([
            ...
            # dcc.Tabs(id="tabs-style1", children=[
            #     dcc.Tab(label='Breakfast Analytics', value='tab-1', ),
            #     dcc.Tab(label='Lunch Analytics', value='tab-2', ),
            #     dcc.Tab(label='Dinner Analytics', value='tab-3', )
            # ], colors={
...
def func(n_clicks):
    pass
    # return dcc.send_data_frame(df.to_excel, "Posssibility Dash Apps_ _Analytics.xlsx", sheet_name="Posssibility Dash Apps Analytics 1")
...
@app.callback(
    ...
    [Input(component_id='rd1', component_property='value'),
     Input(component_id='rd3', component_property='value'),
     Input(component_id='rd5', component_property='value'),
     Input(component_id='rd2', component_property='value'),
     Input(component_id='rd4', component_property='value'),
